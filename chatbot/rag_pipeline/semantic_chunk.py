...
class Chunking:
    ...
    def semantic_chunk(self,docs): #文意切割
        ...
        for index in indices_above_thresh:
            ...
            combined_ids = list(set(chain.from_iterable(d["ids"] if isinstance(d.get("ids"), list) else [d.get("ids")] for d in group if "ids" in d)))
            
            chunks.append({
                'text': combined_text, 
                'category': combined_categories, # 輸出 category
                'id': combined_ids               # 輸出 id
            })
            start_index = index + 1
        ...
        if start_index < len(sentences): #將剩下最後一部分進行處理
            ...
            combined_ids = list(set(chain.from_iterable(d["ids"] if isinstance(d.get("ids"), list) else [d.get("ids")] for d in sentences[start_index:] if "ids" in d)))
